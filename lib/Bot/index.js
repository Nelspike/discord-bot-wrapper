...
class Bot extends Client {
  ...
  signin(setName) {
    ...
    return this.login.apply(this, loginArgs).then(() => {
      let namePromise = Promise.resolve(true);
  ...
  _applyStatus() {
    ...
    return user.setStatus(this.botStatus, this.game).then(user => {
      return user.status === this.botStatus &&
        (this.game ? user.game.name === this.game : true);
    }).catch(error => {
